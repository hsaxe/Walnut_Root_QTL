...
### Get SNP data
```{r}
M1 = fread('SCRI_ROOT_SNPs_cleaned_RFE_Jm_31.01.csv') 

M2 = fread('SCRI_ROOT_SNPs_cleaned_RFE_Jr_31.01.csv')

M = M1 %>% 
  left_join(M2) %>% 
  mutate(across(everything(), ~ case_when(.x == '' ~ NA,
                                          T ~ .x)))

RNAseq_dat = M %>% 
  select(Hybrid,X31.01_Jm4D_26359154) %>% 
...
# Read in rfe results
```{r}
rfe_res_all = readRDS('rfe_results_NEM_2and3_year_length_2and3_year_CG_PHY_Jm_Jr_31.01.RData')
...
summary(NEM_pheno$RLN_2Y)

summary(NEM_pheno$RLN_3Y)
...
```{r}
NEM_Height_lmm_2Y = lmerTest::lmer(Height_2Y ~ RLN_2Y +
                                  # (1|genotype)+
                                  (1|Female_Parent),
                                data = NEM_pheno)

summary(NEM_Height_lmm_2Y)
```

```{r}
NEM_Height_lmm_3Y = lmerTest::lmer(Height_3Y ~ RLN_3Y +
                                  # (1|genotype)+
                                  (1|Female_Parent),
                                data = NEM_pheno)

summary(NEM_Height_lmm_3Y)
```
...
```{r}
lm_dat_2Y = RLN_2Y$data %>% 
  select(Hybrid) %>% 
  left_join(NEM_pheno,
            by = c('Hybrid' = 'CAL_Wip_ID'))

NEM_Height_lm_2Y = lm(Height_2Y ~ RLN_2Y 
                                  # (1|genotype)+
                                  # (1|Female_Parent)
                                  ,
                                data = lm_dat_2Y)

summary(NEM_Height_lm_2Y)
```
...
### Modified function for Pat's suggestion
```{r}
RF_reps = function(times = 5, data, pheno, desc = F){
...
rf_model <- randomForest(as.formula(paste0(pheno, ' ~ ', '.')), 
                         data = train,
                         importance = TRUE)


# Get importance scores (Mean Decrease in Impurity)
top_vars <- importance(rf_model, type = 1) %>% 
  as.data.frame() %>% 
  rownames_to_column(var = 'Marker') %>% 
  arrange(desc(`%IncMSE`)) %>% 
  slice_head(n = 8) %>% 
  pull(Marker)# type = 1 for MDI
  
  # Independently test top 8 features by training a model using them
  mod = caret::train(as.formula(paste(pheno,
                                      paste(top_vars,
                                            collapse = ' + '),
                                      sep = ' ~ ')),
...
```{r}
rqtl_dat1 = fread('SCRI_ROOT_SNPs_cleaned_rQTL_Jm_31.01.csv')%>% 
  left_join(dplyr::select(NEM_pheno, CAL_Wip_ID, RLN_2Y), 
            by = c('Hybrid' = names(NEM_pheno)[1])) %>% 
  relocate(RLN_2Y)

rqtl_dat2 = fread('SCRI_ROOT_SNPs_cleaned_rQTL_Jr_31.01.csv')%>% 
  left_join(dplyr::select(NEM_pheno, CAL_Wip_ID, RLN_2Y), 
            by = c('Hybrid' = names(NEM_pheno)[1])) %>% 
  relocate(RLN_2Y)
...
fwrite(rbind(Chromosomes, RLN_2Y$rqtl_dat,
                    fill = T), 'R_qtl_data/R_qtl_RLN_2Y_data_Jm_Jr_31.01.csv')
```
...
RLN_2Y$cross = read.cross(format = "csv", 
                          dir = getwd(), 
                          file = 'R_qtl_data/R_qtl_RLN_2Y_data_Jm_Jr_31.01.csv', 
                          genotypes=c('a', 'b'),
                          alleles=c('a', 'b'))
```
...
```{r}
lm_dat_3Y = RLN_3Y$data %>% 
  select(Hybrid) %>% 
  left_join(NEM_pheno,
            by = c('Hybrid' = 'CAL_Wip_ID'))

NEM_Height_lm_3Y = lm(Height_3Y ~ RLN_3Y 
                                  # (1|genotype)+
                                  # (1|Female_Parent)
                                  ,
                                data = lm_dat_3Y)

summary(NEM_Height_lm_3Y)
```
...
fwrite(rbind(Chromosomes, RLN_3Y$rqtl_dat,
                    fill = T), 'R_qtl_data/R_qtl_RLN_3Y_data_Jm_Jr_31.01.csv')
```
...
RLN_3Y$cross = read.cross(format = "csv", 
                          dir = getwd(), 
                          file = 'R_qtl_data/R_qtl_RLN_3Y_data_Jm_Jr_31.01.csv', 
                          genotypes=c('a', 'b'),
                          alleles=c('a', 'b'))
```
...
fwrite(rbind(Chromosomes, Height_2Y$rqtl_dat,
                    fill = T), 'R_qtl_data/R_qtl_Height_2Y_data_Jm_Jr_31.01.csv')
```
...
Height_2Y$cross = read.cross(format = "csv", 
                          dir = getwd(), 
                          file = 'R_qtl_data/R_qtl_Height_2Y_data_Jm_Jr_31.01.csv', 
                          genotypes=c('a', 'b'),
                          alleles=c('a', 'b'))
```
...
fwrite(rbind(Chromosomes, Height_3Y$rqtl_dat,
                    fill = T), 'R_qtl_data/R_qtl_Height_3Y_data_Jm_Jr_31.01.csv')
```
...
Height_3Y$cross = read.cross(format = "csv", 
                          dir = getwd(), 
                          file = 'R_qtl_data/R_qtl_Height_3Y_data_Jm_Jr_31.01.csv', 
                          genotypes=c('a', 'b'),
                          alleles=c('a', 'b'))
```
...
fwrite(rbind(Chromosomes, CG_Avg$rqtl_dat,
                    fill = T), 'R_qtl_data/R_qtl_CG_Avg_data_Jm_Jr_31.01.csv')
```
...
CG_Avg$cross = read.cross(format = "csv", 
                          dir = getwd(), 
                          file = 'R_qtl_data/R_qtl_CG_Avg_data_Jm_Jr_31.01.csv', 
                          genotypes=c('a', 'b'),
                          alleles=c('a', 'b'))
```
...
fwrite(rbind(Chromosomes, PHY_Avg$rqtl_dat,
                    fill = T), 'R_qtl_data/R_qtl_PHY_Avg_data_Jm_Jr_31.01.csv')
```
...
PHY_Avg$cross = read.cross(format = "csv", 
                          dir = getwd(), 
                          file = 'R_qtl_data/R_qtl_PHY_Avg_data_Jm_Jr_31.01.csv', 
                          genotypes=c('a', 'b'),
                          alleles=c('a', 'b'))
```
...
# rfe_res_all
# 
# saveRDS(rfe_res_all, 'rfe_results_NEM_2and3_year_length_2and3_year_CG_PHY_Jm_Jr_31.01.RData')
```
...
# saveRDS(all_res, 'All_Traits_QTL_Selection_Results_Jm_Jr_31.01_no_RFE.RData')

```

```{r}
all_res = readRDS('All_Traits_QTL_Selection_Results_Jm_Jr_31.01_no_RFE.RData')
...
```{r}
thresholds = lapply(all_res, FUN = function(x){summary(x$perm)[1] %>% sprintf('%.1f', .)})
...
results_tables$n_Markers_table
...
results_tables$sig_distances_table
```
...
total_var_explained = lapply(all_res, FUN = function(x){
  ...
  lapply(Markers, FUN = function(x){
    ...
    sprintf("%.1f%%",lm_summary$adj.r.squared*100)
...
results_tables$total_var_explained_table
...
res$adj.r.squared = sprintf("%.1f%%",lm_summary$adj.r.squared*100)
...
results_tables$top_var_explained_table
...
fwrite(results_tables$Gen_Stats_Table,
       'Results/Gen_Stats_Table_JmJr_31.01.csv')
...
fwrite(results_tables$Top_Stats_Table,
       'Results/Top_Stats_Table_JmJr_31.01.csv')
...
top_boxplot_dat = lapply(all_res, FUN = function(x){
  
name = print(x$name)  
  
top = x$RFE_RF$rfe$variables$var[1:4]

boxplot_dat = x$data %>% 
  pivot_longer(!c(Hybrid,
                  name),
               names_to = 'Marker',
               values_to = 'Genotype') %>% 
  mutate(Genotype = sprintf('%.2f', Genotype) %>% 
           as.factor()) %>% 
  filter(Marker %in% top) %>% 
  pivot_longer(name,
               names_to = 'Trait',
               values_to = 'Trait Value')
})

top_boxplot_dat = do.call(rbind, top_boxplot_dat) %>%
  as_tibble() %>% 
  mutate(Marker = gsub('X31.01_', '', Marker),
...
SNPs_plot = ggplot(top_boxplot_dat,
       aes(Genotype, `Trait Value`))+
  geom_boxplot()+
  ggtitle(label = 'Top Markers 31.01 x SerrTop Markers 31.09 x Serr\nTrait on y-axis and Genotype on x-axis')+
  theme(strip.text = element_markdown(face = 'bold'),
        plot.title = element_text(hjust = 0.5,
                                  face = 'bold'))+
  theme(strip.text = element_markdown(face = 'bold'))+
  facet_wrap(Trait~Marker,
             scales = 'free',
             ncol = 4)
...
### Save plot
```{r}
save_plot(fig = SNPs_plot,
          filename = 'Results/SNPs_boxplot_31.01.png',
          width = 20,
          height = 25)
```
...
write.xlsx(Model_performances,
           file = 'Results/prediction_selection_performances_Jm_Jr_31.01.xlsx',
           borders = 'rows',
           headerStyle = hs,
...
caption = data.frame(Description = 'This file contains the top markers generated by the RF modeling approach. Marker is the name of the SNP marker. Chromosome is the chromosome. Position is the position in base pairs. Replicates are the results in MDI for each marker by each replicate.')

addWorksheet(top_markers_RFE_RF_wb, sheetName = "Caption")
...
saveWorkbook(top_markers_RFE_RF_wb,
       'Results/Top_markers_RFE_RF_Jm_Jr_31.01.xlsx',
       overwrite = T)
```
...
caption = data.frame(Description = 'This file contains the top markers generated by the rrBLUP modeling approach. Marker is the name of the SNP marker. Chromosome is the chromosome. Position is the position in base pairs. Replicates are the effect sizes for each marker by each replicate.')

addWorksheet(top_markers_rrBLUP_wb, sheetName = "Caption")
...
saveWorkbook(top_markers_rrBLUP_wb,
       'Results/Top_markers_rrBLUP_Jm_Jr_31.01.xlsx',
       overwrite = T)
```
...
saveWorkbook(top_markers_QTls_wb,
       'Results/Top_markers_RQTls_Jm_Jr_31.01.xlsx',
       overwrite = T)
...
```{r}
manhattans = ggpubr::ggarrange(all_res$RLN_2Y$manhattan_plot,
                              all_res$RLN_3Y$manhattan_plot,
                              all_res$Height_2Y$manhattan_plot,
                              all_res$Height_3Y$manhattan_plot,
                              all_res$CG_Avg$manhattan_plot,
                              all_res$PHY_Avg$manhattan_plot,
                              nrow = 3,
                              ncol = 2)

manhattans

sjPlot::save_plot('Results/manhattan_plots_Jm_Jr_31.01.png',
                  manhattans,
                  width = 35,
                  height = 30)
```
