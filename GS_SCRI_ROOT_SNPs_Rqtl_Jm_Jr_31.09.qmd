...
# saveRDS(all_res, 'All_Traits_QTL_Selection_Results_Jm_Jr_31.09_no_RFE.RData')

```

```{r}
all_res = readRDS('All_Traits_QTL_Selection_Results_Jm_Jr_31.09_no_RFE.RData')
...
results_tables$top_distances_table
```
...
top_boxplot_dat = do.call(rbind, top_boxplot_dat) %>%
  as_tibble() %>% 
  mutate(Marker = gsub('X31.09_', '', Marker),
...
SNPs_plot = ggplot(top_boxplot_dat,
  ...
  geom_boxplot()+
  ggtitle(label = 'Top Markers 31.09 x Serr\nTrait on y-axis and Genotype on x-axis')+
...
save_plot(fig = SNPs_plot,
          filename = 'Results/SNPs_boxplot_31.09.png',
          width = 20,
          height = 25)
```
