...
Perf$both = rbind(Perf$perf_01, Perf$perf_09) %>% 
  mutate(Trait = case_when(Trait == 'CG_Avg' ~ '<i>A. tumefaciens</i><br>Disease Score',
...
Stats_Tables_31.01$TSTkable = kable(Stats_Tables_31.01$TST1, 
  ...
  # column_spec(1:length(colnames(Stats_Tables_31.01$TST1)), width = '1em') %>%
  kable_styling(bootstrap_options = c("striped",
                                      "hover",
                                      "condensed",
                                      "responsive"),
...
webshot2::webshot(url = "Results/Top_Stats_table_JmJr_31.09.html", 
        ...
        zoom = 2,
        cliprect = c(135, 5, 660, 260))

```

# Convert this to .R for (hopefully) faster execution
```{r}
knitr::purl('Prediction_performance_figures.qmd', 
            output = 'Prediction_performance_figures.R')
```
