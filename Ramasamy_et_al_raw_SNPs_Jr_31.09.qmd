...
## Load SNP data
```{r}
SNPs = read.csv("SCRI_Root_SNP_Jr_31.09.csv")
...
fwrite(SNPs2, 'R_qtl_SCRI_Root_SNPs_Jr_31.09.csv')

# weirdSNP = SNPs %>% select(starts_with('X31.09_Jm6D_11'))
```
...
fwrite(M, 'SCRI_ROOT_SNPs_cleaned_rQTL_Jr_31.09.csv')
```
...
fwrite(M, 'SCRI_ROOT_SNPs_cleaned_RFE_Jr_31.09.csv')
```
...
```{r}
M = fread('SCRI_ROOT_SNPs_cleaned_Jr_31.09.csv')
